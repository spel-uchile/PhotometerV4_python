...
void setup()
{
  ...
  int_minute = Clock.getMinute();
  min_delta = int_minute/5;
  if (min_delta == 11)
  {
    min_deltap = 0;
  }
  else if (min_delta == 12)
  {
    min_deltap = 5;
  }
  else
  {
    min_deltap = 5*min_delta+5;
  }
  
  A1Day = byte(14);
  A1Hour = byte(0);
  A1Minute = byte(min_deltap);  // si coloca 0 la alarma se activara en 10, si no entiende lea la biblia
  A1Second = byte(0);
  AlarmBits = B11100;
  A1Dy = false;
  A1h12 = false;
...
void loop()
{
  ...
  // GRAN AVENIDA
  //float latitude = -33.522805;
  //float longitude = -70.649178;
  //float timezone = 0;
  // Valle Nevado
...
//********************************************************************************************************************************************
  ...
  myservo1.writeMicroseconds(sec(-30));
  delay(500);
...
// Eliminamos cuando el valor es de 2
      ...
      if ((hor0 > hor+45) or (hor0 < hor-45) )
      {
        hor0 = hor;
      }
    
      if ((ver0 > ver+45) or (ver0 < ver-45) )
      {
        ver0 = ver;
      }
