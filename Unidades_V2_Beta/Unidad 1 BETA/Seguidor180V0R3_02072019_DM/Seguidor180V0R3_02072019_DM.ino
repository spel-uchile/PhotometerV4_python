...
// Costantes calculo posición del sol
...
void setup()
{
 ...
 // Estado inicial de alarma
  A1Day = byte(14);
  A1Hour = byte(0);
  A1Minute = byte(05);  // si coloca 0 la alarma se activara en 10, si no entiende lea la biblia
  A1Second = byte(0);
  AlarmBits = B11100;
  A1Dy = false;
  A1h12 = false;
  ...
  // Selecion de alarma y activacion inmediata
...
void loop()
{
  ...
  delay(100);
  Serial.println("Proximo despertar en 5 minutos");
  delay(100);
  
  // Set de la alarma próximo reinicio
  Clock.setA1Time(A1Day, A1Hour, A1Minute, A1Second, AlarmBits, A1Dy, A1h12, A1PM);
  ...
  if (altitude >= 7)
  {
    ...
    while (millis() < tiempo + 120000)
    {
      ...
      //Identificacion de angulos objetivos
      hor0 = hor0 + x;
      ver0 = ver0 + y;


      // Correccion en caso de divergencia
    ...
    // Esperamos 90 seg para que tome datos del GPS y escriba en la SD y apagamos
    delay(90000);
    digitalWrite(7, LOW);
  }
  Serial.println("OK");
