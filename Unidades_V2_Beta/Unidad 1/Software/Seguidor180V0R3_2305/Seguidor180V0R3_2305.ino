...
int optico1 = 12; //*** CUIDADO SE DEBEN REASIGNAR SEGUN CUAL SEA EL OPTICO ASIGNADO AL PIN ***
int optico2 = 11; // OPTICO1 = SENSOR VERTICAL ; OPTICO2 = SENSOR HORIZONTAL
int amp=2;
...
void setup()
{
  ...
  A1Day = byte(14);
  A1Hour = byte(0);
  A1Minute = byte(45);  // si coloca 0 la alarma se activara en 10, si no entiende lea la biblia
  A1Second = byte(0);
  AlarmBits = B11100;
  A1Dy = false;
  A1h12 = false;
...
//START OF THE CODE THAT CALCULATES THE POSITION OF THE SUN
  ...
  myservo1.write(int(correctaz));
  myservo2.write(int(correctal));
...
//********************************************************************************************************************************************
  ...
  Serial.println(digitalRead(optico1));
  Serial.println(digitalRead(optico2));
  
  //if (digitalRead(optico2 == 0)) 
  //{
  int a = 6;
  bool test_found = false;

  // Levantar el seguidor
  myservo1.write(00);
  myservo2.write(180);
  delay(100);
  ...
  // Algoritmo para la busqueda del sol
  for (int i=1; i<=6;i++)
  {
    myservo1.write(100);
    delay(200);
  ...
  */
   ...
   //}  
  //if (digitalRead(optico2) == 1)

  // Seguimiento de presicion
...
// Eliminamos cuando el valor es de 2
      ...
      if ((hor0 >= 180) or (hor0 <= 0) )
      {
        hor0 = hor;
      }
    
      if ((ver0 >= 180) or (ver0 <= 0) )
      {
        ver0 = ver;
      }
